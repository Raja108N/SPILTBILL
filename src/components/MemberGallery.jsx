    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            ...
            <div className="flex items-center justify-between p-6">
                <button onClick={onBack} className="btn-secondary text-sm flex items-center gap-2 group">
                    <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to Dashboard
                </button>
                <div className="text-right">
                    <h2 className="text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">{member?.name}</h2>
            ...
            <div className="px-6 pb-6">
                <div className="glass-panel p-6 flex flex-col items-center justify-center relative overflow-hidden">
                    ...
                    <span className="text-5xl font-bold font-mono text-white relative z-10 text-shadow-glow">£{totalSpent.toFixed(2)}</span>
                ...
                {memberReceipts.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {memberReceipts.map(receipt => (
                            <div key={receipt.id} className="glass-panel group overflow-hidden hover:border-primary/50 transition-all hover:shadow-glow">
                                {/* Image / Placeholder */}
                                <div className="h-48 bg-surface-hover relative overflow-hidden">
                                    {receipt.image ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center bg-white/5">
                                            <Receipt size={48} className="text-white/20 group-hover:text-primary transition-colors" />
                                        </div>
                                    )}
                                ...
                                {/* Details */}
                                <div className="p-4">
                                    <h3 className="font-bold text-lg mb-1 truncate" title={receipt.description}>{receipt.description || 'Expense'}</h3>
                                    ...
                                    {/* Split info pill */}
                                    <div className="inline-flex items-center gap-1 bg-white/10 rounded-full px-2 py-1 text-xs text-muted/80">
